...
from vit_pytorch.positionalembedding2d import positionalencoding2d
...
class ViTV2(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, channels = 3, attn_dropout = 0., ff_dropout = 0.):
        ...
        #self.pos_embedding = nn.Parameter(torch.randn(1, num_patches + 1, dim))
        pe = positionalencoding2d(dim, image_size // patch_size, image_size // patch_size).cuda()
        pe = pe.reshape(dim,pe.shape[1]*pe.shape[2]).permute(1,0).unsqueeze(0)
        pe = torch.cat([pe,torch.zeros_like(pe)[:,:1]], axis=1)
